...
this.path('b lw %',prop.linewidth);if(prop.shadow){RGraph.setShadow({object:this,prefix:'shadow'});}
this.path('b a % % % % % false f % s %',this.coords[0][0],this.coords[0][1],this.radius,0,RGraph.TWOPI,prop.colorsFill,prop.colorsStroke);RGraph.noShadow(this);RGraph.installEventListeners(this);if(this.firstDraw){this.firstDraw=false;RGraph.fireCustomEvent(this,'onfirstdraw');this.firstDrawFunc();}
RGraph.fireCustomEvent(this,'ondraw');return this;};this.exec=function(func)
{func(this);return this;};this.getObjectByXY=function(e)
{if(this.getShape(e)){return this;}};this.getShape=function(e)
{var mouseXY=RGraph.getMouseXY(e),mouseX=mouseXY[0],mouseY=mouseXY[1];if(RGraph.getHypLength(this.centerx,this.centery,mouseXY[0],mouseXY[1])<=this.radius){if(RGraph.parseTooltipText&&prop.tooltips){var tooltip=RGraph.parseTooltipText(prop.tooltips,0);}
...
{if(prop.tooltipsHighlight){if(typeof prop.highlightStyle==='function'){(prop.highlightStyle)(shape);}else{this.path('b a % % % % % false f % s %',this.centerx,this.centery,this.radius+0.5,0,RGraph.TWOPI,prop.highlightFill,prop.highlightStroke);}}};this.parseColors=function()
